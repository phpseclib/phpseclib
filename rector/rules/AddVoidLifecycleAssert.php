...
final class AddVoidLifecycleAssert extends AbstractRector
{
    ...
    public function refactor(Node $node): ?Node
    {
        ...
        if (
          (in_array($methodName, self::LIFECYCLE_METHODS, true) && count($node->params) === 0) ||
          (in_array($methodName, self::ASSERT_METHODS, true) && count($node->params) > 0)
        ) {
            return $this->addVoidReturn($node);
        }

        return null;
    }

    private function addVoidReturn(ClassMethod $node): ClassMethod
    {
