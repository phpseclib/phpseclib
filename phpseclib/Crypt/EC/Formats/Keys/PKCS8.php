...
abstract class PKCS8 extends Progenitor
{
    ...
    public static function load($key, ?string $password = null): array
    {
        ...
        if (!$params) {
            throw new \phpseclib3\Exception\RuntimeException('Unable to decode the parameters using Maps\ECParameters');
        }
        ...
        if (isset($key['parameters']) && $params != $key['parameters']) {
            throw new \phpseclib3\Exception\RuntimeException('The PKCS8 parameter field does not match the private key parameter field');
        }
    ...
    private static function loadEdDSA(array $key): array
    {
        ...
        if (isset($key['privateKey'])) {
            ...
            if (substr($key['privateKey'], 0, 2) != "\x04\x20") {
                throw new \phpseclib3\Exception\RuntimeException('The first two bytes of the private key field should be 0x0420');
            }
