...
abstract class AsymmetricKey
{
    ...
    public static function load($key, $password = false): AsymmetricKey
    {
        ...
        foreach (self::$plugins[static::ALGORITHM]['Keys'] as $format) {
            ...
            try {
                $components = $format::load($key, $password ?: false);
            } catch (\Throwable $e) {
                $components = false;
            }
            if ($components !== false) {
        ...
        if ($components === false) {
            throw new NoKeyLoadedException('Unable to read key: ' . $e->__toString());
        }
     ...
     * Load Plugins
     */
    private static function loadPlugins(string $format)
    {
     ...
     *
     * @see AsymmetricKey::load()
     * @return void
     */
    public static function addFileFormat(string $fullname)
    {
        self::initialize_static_variables();
