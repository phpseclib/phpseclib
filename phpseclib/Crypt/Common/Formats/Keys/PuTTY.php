...
abstract class PuTTY
{
    ...
    public static function load(array|string $key, ?string $password): array
    {
        ...
        if (str_contains($key, 'BEGIN SSH2 PUBLIC KEY')) {
            ...
            $components = ([static::PUBLIC_HANDLER, 'load'])($data) + $values;
        ...
        if (Strings::shift($key[0], strlen('PuTTY-User-Key-File-')) != 'PuTTY-User-Key-File-') {
            throw new UnexpectedValueException('PuTTY private keys need to begin with "PuTTY-User-Key-File-"');
        }
        ...
        if ($newtype != $type) {
            throw new UnexpectedValueException('The binary type does not match the human readable type field');
        }
