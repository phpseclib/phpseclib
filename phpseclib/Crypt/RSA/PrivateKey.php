...
class PrivateKey extends RSA implements Common\PrivateKey
{
    ...
    private function rsadp(BigInteger $c)
    {
        if ($c->compare(self::$zero) < 0 || $c->compare($this->modulus) > 0) {
            throw new \phpseclib3\Exception\OutOfRangeException('Ciphertext representative out of range');
        }
    ...
    private function rsasp1(BigInteger $m)
    {
        if ($m->compare(self::$zero) < 0 || $m->compare($this->modulus) > 0) {
            throw new \phpseclib3\Exception\OutOfRangeException('Signature representative out of range');
        }
    ...
    private function rsaes_pkcs1_v1_5_decrypt(string $c)
    {
        ...
        if (strlen($c) != $this->k) { // or if k < 11
            throw new \phpseclib3\Exception\LengthException('Ciphertext representative too long');
        }
        ...
        if (ord($em[0]) != 0 || ord($em[1]) > 2) {
            throw new \phpseclib3\Exception\RuntimeException('Decryption error');
        }
    ...
    public function getPublicKey(): RSA
    {
        ...
        if (empty($this->modulus) || empty($this->publicExponent)) {
            throw new \phpseclib3\Exception\RuntimeException('Public key components not found');
        }
