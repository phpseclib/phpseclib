...
abstract class ASN1
{
    ...
    public static function extractBERAsString(string $pem): string
    {
        self::loadAllOIDs();
        $der = self::extractBER($pem);
        $decoded = self::decodeBER($der);
        return self::unmap($decoded);
    }

    public static function unmap(array $decoded): string
    {
        if (!isset($decoded['content']) || !$decoded['content'] instanceof Constructed) {
            throw new InvalidArgumentException('The array passed to ASN1::unmap() should come from ASN1::decodeBER()');
        }
        $decoded['content'] = $decoded['content']->toArray();
        return self::makePrintable($decoded, 0, 0);
    }

    // based off of https://github.com/phpseclib/docs/blob/master/x509/asn1parse.php
    private static function makePrintable(array $decoded, int $depth, int $start): string
    {
        $output = '';

        if (isset($decoded['length'])) {
            $length = $decoded['length'] - $decoded['headerlength'];
        } else {
//print_r(array_keys($decoded)); exit;
//            throw new RuntimeException('Indefinite lengths are not currently supported');
            $length = ' inf';
            $decoded['headerlength'] = 2;
        }

        if (isset($decoded['constant'])) {
            $constructed = is_array($decoded['content'][0]);
            $output.= self::addLine($start, $depth, $length, $decoded['headerlength'], $constructed, 'cont [ ' . $decoded['constant'] . ' ]');
            if ($constructed) {
                $start+= $decoded['headerlength'];
                foreach ($decoded['content'] as $item) {
                    $output.= self::makePrintable($item, $depth + 1, $start);
                    return $output;
                }
            }
            return $output;
        ...
        switch ($decoded['type']) {
            case self::TYPE_SEQUENCE:
            case self::TYPE_SET:
                $type = $decoded['type'] == self::TYPE_SEQUENCE ? 'SEQUENCE' : 'SET';
                $output.= self::addLine($start, $depth, $length, $decoded['headerlength'], true, $type);
                $start+= $decoded['headerlength'];
                foreach ($decoded['content'] as $item) {
                    $output.= self::makePrintable($item, $depth + 1, $start);
if (!isset($item['length'])) {
//    print_r(array_keys($item)); exit;
}
                    $start+= $item['headerlength'] + ($item['length'] ?? $item['actuallength']);
                }
                return $output;
            case ASN1::TYPE_INTEGER:
if (is_string($decoded['content'])) {
    // should be:
    // 12108:d=7  hl=2 l=   1 prim: INTEGER           :02
    // is:
    //    49:d=5  hl=2 l=   1 prim: ZZZZ INTEGER      :XXX
    return self::addLine($start, $depth, $length, $decoded['headerlength'], false, 'ZZZZ INTEGER', 'XXX');
    echo "start = $start\n";
    var_dump($decoded['content']);
    exit;
}
                $value = $decoded['content']->toHex(true);
                if (empty($value)) {
                    $value = '00';
                }
                return self::addLine($start, $depth, $length, $decoded['headerlength'], false, 'INTEGER', strtoupper($value));
            case self::TYPE_OBJECT_IDENTIFIER:
                return self::addLine($start, $depth, $length, $decoded['headerlength'], false, 'OBJECT', (string) $decoded['content']);
            case self::TYPE_BIT_STRING:
                return self::addLine($start, $depth, $length, $decoded['headerlength'], false, 'BIT STRING');
            case self::TYPE_BOOLEAN:
                return self::addLine($start, $depth, $length, $decoded['headerlength'], false, 'BOOLEAN', (string) ord($decoded['content']->getEncodedWithoutHeader()));
            case self::TYPE_OCTET_STRING:
                if (is_array($decoded['content'])) {
                    $output.= self::addLine($start, $depth, 'inf', $decoded['headerlength'], true, 'OCTET STRING');
                    $start+= $decoded['headerlength'];
                    foreach ($decoded['content'] as $item) {
                        $output.= self::makePrintable($item, $depth + 1, $start);
                        $start+= $item['headerlength'] + $item['length'];
                    }
                    return $output;
//    print_r($decoded['content']); exit;
                }
                return self::addLine($start, $depth, $length, $decoded['headerlength'], false, 'OCTET STRING', strtoupper(bin2hex($decoded['content']->getEncodedWithoutHeader())));
            case self::TYPE_NULL:
                return self::addLine($start, $depth, $length, $decoded['headerlength'], false, 'NULL');
            case self::TYPE_NUMERIC_STRING:
                $type = 'NUMERICSTRING';
            case self::TYPE_PRINTABLE_STRING:
            ...
            case self::TYPE_BMP_STRING:
                $type ?? $type = 'BMPSTRING';
                return self::addLine($start, $depth, $length, $decoded['headerlength'], false, $type, (string) $decoded['content']);
            case self::TYPE_GENERALIZED_TIME:
            case self::TYPE_UTC_TIME:
                $type = $decoded['type'] == self::TYPE_GENERALIZED_TIME ? 'GENERALIZEDTIME' : 'UTCTIME';
                return self::addLine($start, $depth, $length, $decoded['headerlength'], false, $type, $decoded['content']->getEncodedWithoutHeader());
        }
        if ($decoded['type'] === 0 && $length === -2) {
            return self::addLine($start, $depth, $length, $decoded['headerlength'], false, 'EOC');
        }
        throw new RuntimeException($decoded['type'] . ' is not a supported tag');
    }

    public static function addLine(int $start, int $depth, int|string $length, int $headerlength, bool $constructed, string $type, ?string $extra = null): string
    {
        if (is_int($length)) {
            $length+= $headerlength;
        }
        $output = str_pad("$start", 5, ' ', STR_PAD_LEFT) . ':' .
                  'd=' . str_pad("$depth", 3) .
                  'hl=' . str_pad("$headerlength", 2) .
                  'l=' . str_pad("$length", 4, ' ', STR_PAD_LEFT) . ' ' .
                  ($constructed ? 'cons' : 'prim') . ': ' .
                  //str_repeat(' ', $depth) . $type;
                  $type;
        if (isset($extra)) {
            $extra = ':' . $extra;
            if ($type == 'OCTET STRING') {
                $extra = '[HEX DUMP]' . $extra;
            }
            $output.= str_repeat(' ', 18 - strlen($type)) . $extra;
        }
        return "$output\r\n";
    }
